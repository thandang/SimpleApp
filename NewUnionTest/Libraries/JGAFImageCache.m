...
//    NSAssert(url.length > 0, @"url cannot be nil");
    if (url && [url length]) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            if(image == nil) {
                [weakSelf loadRemoteImageForURL:url key:sha1 retryCount:3 completion:completion];
            }
            else if(completion) {
...
//            if(image == nil) {
//                image = [weakSelf imageFromDiskForKey:sha1];
//                DEBUG_LOG(@"log image 1");
//            }
//        });
        
        NSString *sha1 = [url jgaf_sha1];
        UIImage *image = [self imageFromDiskForKey:sha1];
        return image;
    }
    return nil;
}
...
#if JGAFImageCache_LOGGING_ENABLED
                        NSLog(@"%s [Line %d] retrying(%ld)", __PRETTY_FUNCTION__, __LINE__, (long)nextRetryCount);
#endif
                    }
                    
#if JGAFImageCache_LOGGING_ENABLED
                    NSLog(@"%s [Line %d] statusCode(%ld) %@", __PRETTY_FUNCTION__, __LINE__, (long)httpStatusCode, error);
#endif
                } break;
            }
        }];
